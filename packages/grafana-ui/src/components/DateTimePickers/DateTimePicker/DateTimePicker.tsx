    ...
    return (
      <InlineField label={label} invalid={!!(internalDate.value && internalDate.invalid)} className={styles.field}>
        <Input
          ...
          placeholder={t('grafana-ui.date-time-picker.select-placeholder', 'Выберите дату/время')}
          ref={ref}
          suffix={
            clearable &&
      ...
      <div className={cx(styles.container, { [styles.fullScreen]: isFullscreen })} style={style} ref={ref}>
        ...
        <Stack>
          <Button type="button" onClick={handleApply}>
            <Trans i18nKey="grafana-ui.date-time-picker.apply">Применить</Trans>
          </Button>
          <Button variant="secondary" type="button" onClick={onClose}>
            <Trans i18nKey="grafana-ui.date-time-picker.cancel">Отмена</Trans>
          </Button>
        </Stack>
      </div>
    );
