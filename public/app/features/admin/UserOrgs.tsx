          ...
          <div>
            {canAddToOrg && (
              <Button variant="secondary" onClick={this.showOrgAddModal} ref={this.addToOrgButtonRef}>
                <Trans i18nKey="admin.user-orgs.add-button">Добавить в организацию</Trans>
              </Button>
            )}
          </div>
          <AddToOrgModal
      ...
      <tr>
        ...
        <td colSpan={1}>
          {canRemoveFromOrg && (
            ...
            >
              {t('admin.user-orgs.remove-button', 'Удалить из организации')}
            </ConfirmButton>
          )}
        </td>
      </tr>
        ...
        <Modal.ButtonRow>
          <Stack gap={2} justifyContent="center">
            <Button variant="secondary" fill="outline" onClick={this.onCancel}>
              <Trans i18nKey="admin.user-orgs-modal.cancel-button">Отмена</Trans>
            </Button>
            <Button variant="primary" disabled={selectedOrg === null} onClick={this.onAddUserToOrg}>
              <Trans i18nKey="admin.user-orgs-modal.add-button">Добавить в организацию</Trans>
            </Button>
          </Stack>
        </Modal.ButtonRow>
      </Modal>
  ...
  return (
    <div className={styles.disabledTooltip}>
      {isExternalUser ? (
        ...
        >
          {t('admin.user-orgs.change-role-button', 'Изменить роль')}
        </ConfirmButton>
      )}
    </div>
  );
