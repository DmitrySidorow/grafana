  ...
  return (
    <div>
      <h3 className="page-heading">
        <Trans i18nKey="admin.user-profile.title">Информация о пользователе</Trans>
      </h3>
      <Stack direction="column" gap={1.5}>
        <div>
          <table className="filter-table form-inline">
            <tbody>
              <UserProfileRow
                label={t('admin.user-profile.label-numerical-identifier', 'ID')}
                value={user.id.toString()}
                locked={true}
              />
              <UserProfileRow
                label={t('admin.user-profile.label-name', 'ФИО')}
                value={user.name}
                locked={editLocked}
              ...
              />
              <UserProfileRow
                label={t('admin.user-profile.label-username', 'Логин')}
                value={user.login}
                locked={editLocked}
                lockMessage={lockMessage}
              ...
              />
              <UserProfileRow
                label={t('admin.user-profile.label-password', 'Пароль')}
                value="********"
                inputType="password"
        ...
        <Stack gap={2}>
          {canDelete && (
            <>
              <Button variant="destructive" onClick={showDeleteUserModal(true)} ref={deleteUserRef}>
                <Trans i18nKey="admin.user-profile.delete-button">Удалить пользователя</Trans>
              </Button>
              <ConfirmModal
          ...
          {user.isDisabled && canEnable && (
            <Button variant="secondary" onClick={handleUserEnable}>
              <Trans i18nKey="admin.user-profile.enable-button">Активировать пользователя</Trans>
            </Button>
          )}
          {!user.isDisabled && canDisable && (
            <>
              <Button variant="secondary" onClick={showDisableUserModal(true)} ref={disableUserRef}>
                <Trans i18nKey="admin.user-profile.disable-button">Отключить пользователя</Trans>
              </Button>
              <ConfirmModal
    ...
    return (
      <tr>
        ...
        <td>
          ...
          >
            {t('admin.user-profile.edit-button', 'Редактировать')}
          </ConfirmButton>
        </td>
      </tr>
    );
