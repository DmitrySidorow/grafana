...
class UserSessions extends PureComponent<Props> {
  render() {
    ...
    if (isLoading) {
      return <LoadingPlaceholder text={<Trans i18nKey="user-sessions.loading">Загрузка сессий...</Trans>} />;
    }
            ...
            <ScrollContainer overflowY="visible" overflowX="auto" width="100%">
              <table className="filter-table form-inline" data-testid={selectors.components.UserProfile.sessionsTable}>
                <thead>
                  <tr>
                    <th>
                      <Trans i18nKey="user-session.seen-at-column">Последний вход</Trans>
                    </th>
                    <th>
                      <Trans i18nKey="user-session.created-at-column">Зарегистрирован</Trans>
                    </th>
                    <th>
                      <Trans i18nKey="user-session.ip-column">IP адрес</Trans>
                    </th>
                    <th>
                      <Trans i18nKey="user-session.browser-column">Браузер & ОС</Trans>
                    </th>
                    <th>
                ...
                <tbody>
                  {sessions.map((session: UserSession, index) => (
                    <tr key={index}>
                      {session.isActive ? (
                        <td>
                          <Trans i18nKey="profile.user-sessions.now">Сейчас</Trans>
                        </td>
                      ) : (
                      ...
                      <td>
                        {session.browser} в {session.os} {session.osVersion}
                      </td>
                      <td>
